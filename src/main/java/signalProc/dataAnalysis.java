...
/*
	public static double[] variance(List<Integer> data, double[] mean, int segundos) {
		...
		for (int i = 0; i < size; i++) {
			if (i>60 ||i<80)System.out.println("data: "+data.get(i)+"mean: "+mean[j]); 

			var += Math.pow(data.get(i) - mean[j], 2);
			number++;
